...
public class REKSolver implements LeastSquaresSolver {
    ...
    public DoubleVector solve(DoubleMatrix A, DoubleVector b, double maxSeconds) {
        ...
        DoubleVector rowProbs = A.rowProbs(), colProbs = A.columnProbs();
        AliasMethod rowSampler = new AliasMethod(rowProbs.asList());
        AliasMethod colSampler = new AliasMethod(colProbs.asList());

        while ( k % CheckEvery != 0 || !timeIsUp(initTime, maxSeconds) ) {
            i_k = rowSampler.next();
            j_k = colSampler.next();

            val = -z.DDOT(A.getColumn(j_k)) / colProbs.get(j_k);       // val = - dot(z, A(:, j_k)) / colProbs(j_k) 
            z.DAXPY(val, A.getColumn(j_k));                             // z = z + val * A(:, j_k);

            val = x.DDOT(A.getRow(i_k));                                // val = dot(x, A(i_k, :))
            val = (b.get(i_k) - z.get(i_k) - val) / rowProbs.get(i_k);  // val = (b(i_k) - z(i_k) - val) / roProbs(i_k)
