...
void sparseREK(const SMAT * A, double *x, const double *b, double TOL){
    
    unsigned int k, l, i_k, j_k, *rowSampl, *colSampl;
    double val, *z, *rowProb, *colProb, *s = A->sVal;
    int m = A->m, n = A->n, *ir = A->row_ptr, *jc = A->col_ind;
    
    // Store the A transpose for efficient row / column operations
    // CAUTION : If A is very large matrix, the following command might cause 'out of memory' issues.
    ...
    // Sample indices for the next BLOCKSIZE iterations
    mySampler (rowSampl, BLOCKSIZE, asRow);
    mySampler (colSampl, BLOCKSIZE, asCol);
    ...
    for (k = 0; k < MAXITERS; k++){
        ...
        val = 0.0;
        for (l = jc[j_k]; l < jc[j_k + 1]; l++){	// val = <z, A(:, j_k)>
            val += z[ir[l]] * s[l];
        }
