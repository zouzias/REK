...
// Sample indices
  ...
  for (k = 0; k < MAXITERS; k++)
      ...
      else if ((k + 1) % BLOCKSIZE == 0)
	{
      ...
      val = DDOT ((ptrdiff_t *) & m, z, &incx, &(A->val)[j_k * m], &incy);
      val /= colProb[j_k];

      val = -val;
      DAXPY ((ptrdiff_t *) & m, &val, &(A->val)[j_k * m], &incx, z, &incy);

      val2 =
	DDOT ((ptrdiff_t *) & n, x, &incx, &(Atransp->val)[i_k * n], &incy);

      val2 = b[i_k] - z[i_k] - val2;
      val2 /= rowProb[i_k];

      DAXPY ((ptrdiff_t *) & n, &val2, &(Atransp->val)[i_k * n], &incx, x,
	     &incy);
    }
		...
		double TOL)
{
  ...
  size_t m = A->m, n = A->n, *ir = A->row_ptr, *jc = A->col_ind;
